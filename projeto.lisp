...
(defun iniciar()
  (let* ((problema (escolher-problema))
         ...
         (heuristica (if (equal algoritmo 'a) (ler-heuristica)))
         (no (cria-no problema heuristica))
...
(defun executar (algoritmo no heuristica)
  (cond ((equal algoritmo 'bfs) (bfs no 'no-solucaop 'sucessores (operadores)))
        ((equal algoritmo 'dfs) (dfs no 'no-solucaop 'sucessores (operadores) 31))
        ((equal algoritmo 'a) (a no 'no-solucaop 'sucessores (operadores) heuristica))
        (t (format t "Error"))
   )
)
