...
"Tenta gerar novos nós de acordo com a posição no tabuleiro"
  (cond ((> x 7) nil)
        ((> y 7) (novo-sucessor-recursivo no operador (1+ x) 1))
...
"Pesquisa do nó solução para o problema usando Breath First Search (BFS)"
  ...
  (cond ((null lista-aberto) nil)
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'bfs))
                ...
                (cond (no-solucao (car no-solucao))
                  (t (bfs-recursivo no-solucaop sucessores operadores (abertos-bfs (cdr lista-aberto) lista-sucessores) (cons (car lista-aberto) lista-fechado)))
                )
           )
        )
  )
