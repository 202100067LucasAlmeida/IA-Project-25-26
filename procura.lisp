...
(defun bfs-recursivo (no-solucaop sucessores operadores lista-aberto lista-fechado)
  (cond ((null lista-aberto) nil)
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'bfs))
                ...
                (no-solucao (remove nil (mapcar #'(lambda (x) (funcall no-solucaop x)) lista-sucessores))))
                (cond ((null lista-sucessores) (bfs-recursivo no-solucaop sucessores operadores (cdr lista-aberto) (cons (car lista-aberto) lista-fechado)))
                      (no-solucao (list :solucao (car no-solucao)
                                        :fator-ramificacao-media (/ (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
                                                                    (+ (list-length lista-fechado) 1))
                                        :nos-gerados (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
                                        :nos-espandidos (+ (list-length lista-fechado) 1)
                                        :penetrancia (/ (no-profundidade (car no-solucao)) 
                                                     (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado)))
                                  ))
                      (t (bfs-recursivo no-solucaop sucessores operadores 
                      (abertos-bfs (cdr lista-aberto) lista-sucessores) (cons (car lista-aberto) lista-fechado)))
                )
           )
        )
  )
...
(defun dfs-recursivo (no-solucaop sucessores operadores profundidade-max lista-aberto lista-fechado)
  (cond ((null lista-aberto) nil)
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'dfs :profundidade profundidade-max))
                ...
                ;(x (format t "~a~%~%~%" lista-fechado))
                (no-solucao (remove nil (mapcar #'(lambda (x) (funcall no-solucaop x)) lista-sucessores))))
                (cond ((null lista-sucessores) (dfs-recursivo no-solucaop sucessores operadores profundidade-max
                        (cdr lista-aberto) (cons (car lista-aberto) lista-fechado)))
                      (no-solucao (list :solucao (car no-solucao)
                                        :fator-ramificacao-media (/ (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
                                                                    (+ (list-length lista-fechado) 1))
                                        :nos-gerados (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
...
(defun a-recursivo (no-solucaop sucessores operadores heuristica lista-aberto lista-fechado)
  (cond ((null lista-aberto) nil)
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'a* :heuristica heuristica))
                ...
                ;(x (format t "~a~%~%~%" lista-fechado))
                (no-solucao (remove nil (mapcar #'(lambda (x) (funcall no-solucaop x)) lista-sucessores))))
                (cond ((null lista-sucessores) (a-recursivo no-solucaop sucessores operadores heuristica 
                        (ordenar-nos (cdr lista-aberto)) (cons (car lista-aberto) lista-fechado)))
                      (no-solucao (list :solucao (car no-solucao)
                                        :fator-ramificacao-media (/ (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
                                                                    (+ (list-length lista-fechado) 1))
                                        :nos-gerados (+ (list-length lista-sucessores) (list-length lista-aberto) (list-length lista-fechado))
                      ...
                      (t (a-recursivo no-solucaop sucessores operadores heuristica 
                         (ordenar-nos (append (cdr lista-aberto) lista-sucessores)) (cons (car lista-aberto) lista-fechado)))
                )
           )
        )
  )
...
(defun heuristica-2 (tabuleiro)
  ...
  (+ (distancia-ao-centro tabuleiro) (* (pinos-isolados tabuleiro) 3))
)
