...
(defun sucessores (no operadores procura &optional(profundidade))
  ...
  (cond ((null no) nil)
        ((null operadores) nil)
        ((null procura) nil)
        ((equal procura 'a*) (let ((todos-nos-ordenados (apply #'append (mapcar #'(lambda (operador) (novo-sucessor no operador)) operadores))))
                (ordenar-nos (remove-if #'null todos-nos-ordenados))
           )
        )
...
(defun recursive-a (no-solucaop sucessores operadores profundidade-max lista-aberto lista-fechado)
  (cond ((null lista-aberto) nil)
        ((= (no-profundidade (car lista-aberto)) profundidade-max) 
            (recursive-a no-solucaop sucessores operadores profundidade-max (cdr lista-aberto) 
            (cons (car lista-aberto) lista-fechado)))
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'a* profundidade-max))
                ;(x (format t "~a~%~%" lista-sucessores))
                ;(x (format t "~a~%~%" lista-aberto))
                ;(x (format t "~a~%~%~%" lista-fechado))
                (no-solucao (remove nil (mapcar #'(lambda (x) (funcall no-solucaop x)) lista-aberto))))
                (cond (no-solucao (car no-solucao))
                  (t (recursive-a no-solucaop sucessores operadores profundidade-max 
                  (abertos-dfs (cdr lista-aberto) lista-sucessores) (cons (car lista-aberto) lista-fechado)))
                )
           )
        )
