...
(defun sucessores (no operadores procura &key(profundidade)(limite 0))
...
(defun a (no-inicial no-solucaop sucessores operadores &optional (lista-aberto nil) (lista-fechado nil))
  (a-recursivo no-solucaop sucessores operadores (cons no-inicial lista-aberto) lista-fechado)
)

(defun a-recursivo (no-solucaop sucessores operadores lista-aberto lista-fechado)
  (cond ((null lista-aberto) nil)
        (t (let* ((lista-sucessores (funcall sucessores (car lista-aberto) operadores 'a*))
                ;(x (format t "~a~%~%" lista-sucessores))
                ;(x (format t "~a~%~%" lista-aberto))
                ;(x (format t "~a~%~%~%" lista-fechado))
                (no-solucao (remove nil (mapcar #'(lambda (x) (funcall no-solucaop x)) lista-aberto))))
                (cond (no-solucao (car no-solucao))
                  (t (a-recursivo no-solucaop sucessores operadores 
                  (abertos-dfs (cdr lista-aberto) lista-sucessores) (cons (car lista-aberto) lista-fechado)))
                )
           )
        )
